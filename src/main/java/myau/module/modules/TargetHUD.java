...
public class TargetHUD extends Module {
    ...
    private float damage(final ItemStack stack, final Entity attacker, final Entity target) {
        ...
        if (attacker instanceof EntityLivingBase) {
            ...
            int strengthLevel = livingAttacker.getActivePotionEffect(Potion.damageBoost) != null
                    ? livingAttacker.getActivePotionEffect(Potion.damageBoost).getAmplifier() + 1 : 0;
        ...
        if (target instanceof EntityLivingBase) {
            ...
            for (int i = 0; i < 4; i++) {
                totalProtectionLevel += EnchantmentHelper.getEnchantmentLevel(Enchantment.protection.effectId,
                        livingTarget.getCurrentArmor(i));
            }
            totalDamage *= (1 - livingTarget.getTotalArmorValue() * 0.04f);
            totalDamage *= (1 - totalProtectionLevel * 0.04f);

            int resistanceLevel = livingTarget.getActivePotionEffect(Potion.resistance) != null
                    ? livingTarget.getActivePotionEffect(Potion.resistance).getAmplifier() + 1 : 0;
    ...
    public void onRender(Render2DEvent event) {
        ...
        if (this.target != null) {
            ...
            float healthRatio = Math.min(Math.max(
                    RenderUtil.lerpFloat(this.newHealth, this.oldHealth, elapsedTime / 150.0F) / this.maxHealth, 0.0F), 1.0F);
    ...
    public String[] getSuffix() {
        return new String[]{color.getModeString()};
    }
